<table class="wpshop_coupon_definition_table" >
	...
	<tr>
		...
		<td>
			<select name="coupon_receiver[]" id="coupon_receiver"  class="chosen_select" multiple data-placeholder="Choose a customer" >
			<?php
				...
				if ( !empty($users) ) :
				 	foreach( $users as $user ) : 
						...
						?>
						<option value="<?php echo $user->ID; ?>" <?php echo ( (!empty( $coupon_receiver) && is_array($coupon_receiver) && in_array($user->ID, $coupon_receiver)) ? 'selected="selected"' : '' ); ?>><?php echo $name; ?> ( <?php echo $user->user_email; ?>)</option>
					<?php 
					endforeach; 
				endif;
				?>
