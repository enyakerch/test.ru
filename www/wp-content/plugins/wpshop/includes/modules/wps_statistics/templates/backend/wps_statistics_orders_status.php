...
<?php foreach( $orders_status as $status => $count ) : ?>
						{value: <?php echo $count; ?>, color:"<?php echo $colors[$status]; ?>"},
<?php endforeach; ?>	
						];	
...
<ul class="wps_statistics_legend">
	<?php foreach( $orders_status as $status => $count ) : ?>
		<li><span style="background : <?php echo $colors[$status]; ?>;" class="legend_indicator"></span><span><?php _e($payment_status[ $status ], 'wpshop' ); ?> (<?php echo $count; ?>)</span></li>
	<?php endforeach; ?>	
</ul>
<?php else : ?>
