		...
		if ( !empty($data_to_save['post_ID']) && (get_post_type($data_to_save['post_ID']) == WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT) ) {
			if ( !empty($data_to_save[wpshop_products::currentPageCode . '_attribute']) ) {
				...
				if ( empty($data_to_save[wpshop_products::currentPageCode . '_attribute']['varchar']['product_reference']) ) {
					...
					$query = $wpdb->prepare("SELECT value_id FROM ".WPSHOP_DBT_ATTRIBUTE_VALUES_VARCHAR." WHERE value = %s AND entity_id != %d AND entity_type_id = %d", $ref, $data_to_save['post_ID'], wpshop_entities::get_entity_identifier_from_code(WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT));
			...
			if ( !empty($data_to_save[WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT_VARIATION]) ) {
				foreach ( $data_to_save[WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT_VARIATION] as $variation_id => $variation_definition ) {
					...
					update_post_meta($variation_id, '_wps_variation_attached_picture', $attached_picture_id );
			...
			foreach ( $attachments as $attachment ) {
				if ( is_int( strpos( $attachment->post_mime_type, $attachement_type ) ) ) {
					...
					if ( !empty( $attachment_output ) ) {
						$product_attachement_list .= '<li class="' . $li_class . '" ><a href="' . $url . '" ' . $link_option . ' >' . $attachment_output . '</a>' . $attachement_more_informations . ' <a href="#" class="delete_post_thumbnail" id="thumbnail_'.$attachment->ID.'"><i class="wps-icon-trash"></i></a></li>';
...
// 		}
		...
		if ( empty( $productPrice ) && ( empty( $catalog_options ) || empty( $catalog_options[ 'wpshop_catalog_empty_price_behaviour' ] ) ) ) {
		...
		/** Gallery **/
		$tpl_component['PRODUCT_COMPLETE_SHEET_GALLERY'] = self::get_product_complete_sheet_galery( $product_id );
	 ...
	 */
	function get_product_complete_sheet_galery( $pid ) {
		$output = '';
		if( !empty($pid) ) {
...
// 		));
	...
	function display_variation_admin( $head_product ) {
		...
		if ( !empty($variations) && is_array($variations) ) {
			...
			foreach ( $variations as $variation ) {
				...
				$tpl_component['VARIATION_DETAIL_PRICE'] = number_format( $p, 2, '.', '').' '.$productCurrency;
	...
	function get_variation_price_behaviour( $product_into_cart, $product_variations, $head_product_id, $variations_options ) {
		global $wpdb;
		...
		if( !empty($product_variations) ) {
			...
			foreach( $product_variations as $product_variation ) {
				...
				if( !empty($variation_metadata) ) {
					...
					$p_ati = ( ( !empty($price_piloting) && $price_piloting == 'TTC' ) ? $variation_metadata['product_price'] : ( $variation_metadata['product_price'] * ( 1 + ( $vat_rate / 100) ) ) );
	...
	function wps_get_summary_variations_product( $product_id, $the_product, $has_variation ) {
		global $wpdb; global $wpshop_payment;
		$output = '';
		$tpl_component = array();
		/**	Get attribute order for current product	*/
		$product_attribute_order_detail = wpshop_attributes_set::getAttributeSetDetails( get_post_meta($product_id, WPSHOP_PRODUCT_ATTRIBUTE_SET_ID_META_KEY, true)  ) ;
